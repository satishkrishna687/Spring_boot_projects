...
@Repository
public class SquareZoneLEEACHDAOImpl extends RoutingDaoImpl implements
		SquareZoneLEEACHDAOIF {
	...
	public boolean deleteSquareZoneTopology() {
		...
		try {
			deleteFlag = delete(DELETE_SQUAREZONETOPOLOGY_LEEACH__QUERY);
		} catch (Exception e) {
			System.out
	...
	public boolean insertSquareZoneTopology(
		...
		try {
			insertAngularTopology = insertTopologyWithSquareConfig(
					topologyInfoValue, INSERT_SQUAREZONETOPOLOGY_LEEACH_QUERY);
		} catch (Exception e) {
			System.out
	...
	public List<TopologyInfoValue> retriveSquareZoneTopology()
		...
		try {
			topologyInfoValueList = retriveTopologyWithSquareConfig(RETRIVE_SQUARETOPOLOGY_LEEACH_QUERY);
		...
		try {
			if (topologyInfoValue != null && topologyInfoValue.size() > 0) {
				for (TopologyInfoValue topInfoValue : topologyInfoValue) {
					...
					update(UPDATE_SQUAREZONETOPOLOGY_LEEACH_QUERY, map);
					updationFlag = true;
				}
			}
	...
	public List<Integer> retriveDistinctSquareZoneIds() {
		...
		try {
			topologyInfoValueList = retriveZoneIDs(RETRIVE_ZONEIDSQUARETOPOLOGY_LEEACH);
	...
	public boolean updateNodeIdWithLeaderSquareZone(int nodeId, int leaderValue) {
		...
		try {
			if (nodeId > 0 && leaderValue >= 0) {
				...
				update(UPDATE_ZONELEADER_FORNODEID_SQUARETOPOLOGY_LEEACH, map);
				updationFlag = true;
	...
	public List<NodeInfoForSquare> retriveNodeInfoForZoneIDSquareZone(int zoneId) {
		...
		try {
			topologyInfoValueList = retriveTopologyWithSquareConfigLimited(
					RETRIVE_NODEIDS_DETAILS_FOR_ZONEID_SQUARETOPOLOGY_LEEACH, createMapForZoneId(zoneId));
	...
	public int retriveSquareZoneTopologyCount() {
		...
		int noOfRecords = 0;
		try {
			noOfRecords = queryForInt(RETRIVE_SQUARETOPOLOGY_LEEACH_COUNT);
		} catch (Exception e) {
			System.out.println("Class-->SquareZoneDAOImpl:Method-->"
	...
	public List<Integer> retriveZoneLeaders() {
		...
		try {
			nodeIdList = retriveZoneLeaders(RETRIVE_NODEIDS_WHO_ARE_ZONE_LEADERS_SQUARE_LEEACH_TOPOLOGY,
