...
var routingColumns=[
         			{
         				header : 'Energy Between Centrality',
...
Ext.onReady(function () {
	...
	var nodeIdPanel = Ext.create('Ext.form.Panel', {
		title:'Select a Node whose Battery Power Has to be Viewed',
	...
	Ext.define('routingViewModel', {
				extend : 'Ext.data.Model',
				fields : [ 
				             {name:'networkCost', mapping:'networkCost',type:'string'}
				        ],
				idProperty: 'networkCost'
			});
	...
	routingViewStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
			type : 'ajax',
			url :contextPath+'/viewnetworkcosttable.do',
			extraParams:{
		 	nodeId:1
			},
			actionMethods:{
	...
	var routingTableGrid = Ext.create('Ext.grid.Panel', {
		collapsible :true,
		forceFit:true,
		title:'Energy of the Selected Node in mw',
		id : 'routingTableGrid',
