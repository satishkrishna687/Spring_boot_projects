<!DOCTYPE html>
<html class="no-js">
<!--<![endif]-->
<meta http-equiv="content-type" content="text/html;charset=utf-8"/>
<head>
<title>View Data Sets</title>
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/extjs41/resources/css/ext-all.css" />
<script type="text/javascript" >
var contextPath='<%=request.getContextPath()%>';
</script>
<script type="text/javascript" src="<%=request.getContextPath()%>/extjs41/bootstrap.js"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/customjs/viewmedicalrequestsstatusforuser.js"></script>

<style>
#confirmationMessage{
background:yellow;
color:black;
}

</style>
	

</head>


<body>
<jsp:include page="/WEB-INF/jsp/doctormenu.jsp"></jsp:include>


<idiv id="confirmationMessage">

</idiv>



<div class="dataSetContainer">
</div>
</body>
</html>